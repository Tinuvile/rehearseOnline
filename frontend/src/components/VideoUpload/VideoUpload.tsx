...
const VideoUpload: React.FC = () => {
  ...
  const handleUpload = useCallback(async (file: File) => {
    try {
      ...
      // 设置当前视频
      actions.setCurrentVideo(uploadResponse.video);
      ...
      // 处理视频
      const processResponse = await videoApi.processVideo(uploadResponse.video.id);
      
      clearInterval(processInterval);
      setProcessProgress(100);
      
      // 获取分析结果
      const analysisResult = await videoApi.getAnalysisResult(uploadResponse.video.id);
      
      // 更新应用状态
      actions.setCurrentVideo(analysisResult.video);
      actions.setTranscripts(analysisResult.transcripts);
      actions.setActorPositions(analysisResult.actor_positions);
      
      // 获取时间轴数据
      const timelineData = await stageApi.getTimelineData(uploadResponse.video.id);
      actions.setLightingCues(timelineData.lighting_cues);
      actions.setMusicCues(timelineData.music_cues);
        ...
        <Space direction="vertical" size="large" style={{ width: '100%' }}>
          ...
          {processing && (
            <>
              ...
              <Text type="secondary">AI正在提取音频、识别演员位置和分析情感</Text>
            </>
          )}
        </Space>
      </Card>
      ...
      <Space direction="vertical" size="large" style={{ width: '100%', textAlign: 'center' }}>
        ...
        <Dragger {...uploadProps} style={{ padding: '40px 20px' }}>
          ...
          <p className="ant-upload-hint">
            支持常见视频格式（MP4、AVI、MOV等），文件大小不超过500MB
          </p>
        </Dragger>
